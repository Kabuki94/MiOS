<#
.SYNOPSIS  Replace corrupted 37-selinux.sh with clean version + push
.DESCRIPTION
    The file has been corrupted by multiple regex patch layers that left
    orphaned MODNAME blocks after the CLOUDWS_POLICIES array.
    This replaces the ENTIRE file with a clean version containing all 18 policies.

    Run from repo root:
      cd C:\Users\Kabu\OneDrive\Documents\GitHub\CloudWS-bootc
      .\fix-selinux-final.ps1
#>
...
Write-Host "`n  Replacing scripts/37-selinux.sh (complete file)..." -ForegroundColor Cyan

# ── Read the clean replacement from this script's DATA section ──
$selinux = @'
#!/bin/bash
...
[System.IO.File]::WriteAllText(
    (Join-Path (Get-Location) "scripts/37-selinux.sh"),
    $selinux.Replace("`r`n", "`n"),
    [System.Text.UTF8Encoding]::new($false)
)

# Verify
$check = [System.IO.File]::ReadAllText((Resolve-Path "scripts/37-selinux.sh").Path)
$lineCount = ($check -split "`n").Count
$policyCount = ([regex]::Matches($check, 'CLOUDWS_POLICIES\[')).Count
$hasMODNAME = $check -match 'MODNAME'
$endsClean = $check.TrimEnd() -match 'SELinux configuration complete\.\"\s*$'

Write-Host "  ✓ 37-selinux.sh replaced ($lineCount lines, $policyCount policies)" -ForegroundColor Green
if ($hasMODNAME) {
    Write-Host "  ✗ WARNING: MODNAME still present!" -ForegroundColor Red
} else {
    Write-Host "  ✓ No MODNAME references (clean)" -ForegroundColor Green
}
if ($endsClean) {
    Write-Host "  ✓ Ends at 'SELinux configuration complete.' (no orphaned blocks)" -ForegroundColor Green
} else {
    Write-Host "  ⚠ File ending needs review" -ForegroundColor Yellow
}

# ── Also clean 38-vm-gating.sh: strip any appended garbage from previous patches ──
$vmScript = "scripts/38-vm-gating.sh"
if (Test-Path $vmScript) {
    $vm = [System.IO.File]::ReadAllText((Resolve-Path $vmScript).Path)
    # The script should end at the "VM gating" completion message
    # Strip everything after the first occurrence of the completion line
    if ($vm -match '(?s)(.*\[38-vm-gating\] VM gating \+ Hyper-V enhanced session \(xorgxrdp\) configured\.)') {
        $cleanVm = $Matches[1]
        if ($cleanVm.Length -lt $vm.Length) {
            $stripped = $vm.Length - $cleanVm.Length
            [System.IO.File]::WriteAllText(
                (Resolve-Path $vmScript).Path,
                $cleanVm.Replace("`r`n", "`n") + "`n",
                [System.Text.UTF8Encoding]::new($false)
            )
            Write-Host "  ✓ 38-vm-gating.sh cleaned ($stripped bytes of garbage removed)" -ForegroundColor Green
        } else {
            Write-Host "  - 38-vm-gating.sh already clean" -ForegroundColor DarkGray
        }
    }
}
...
if ($doPush -eq 'y') {
    git add scripts/37-selinux.sh scripts/38-vm-gating.sh
    git status --short
    git commit -m "fix: complete 37-selinux.sh replacement (18 policies, no MODNAME)

Replaces corrupted file with clean version:
- 18 policies in CLOUDWS_POLICIES associative array
- No orphaned MODNAME blocks from previous patch attempts
- v2.1.2 additions: bootupd_state, resolved_hook, accountsd_malcontent,
  chcon_macadmin, gdm_session_cache
- Also strips garbage appended to 38-vm-gating.sh"
