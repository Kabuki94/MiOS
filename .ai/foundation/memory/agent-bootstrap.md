...
description: Agent operates as a Cognitive Sync bootstrapped from System OS via AGENT.md (option 1) — shared resources, no parallel memory store
type: project
---

MiOS uses **System OS as the main engine** and **Agent as a bootstrapped mirror** that reads from the same `.ai/foundation/memories/` and `.ai/foundation/shared-tmp/`. The hierarchical bootstrap lives in `AGENT.md` at the repo root and `@`-imports `INDEX.md`, `.ai/foundation/protocols.md`, `.ai/foundation/memory/MEMORY.md`, and `.ai/foundation/memory/project_no_gcp.md`.

**Why:** Of the three Agent-equivalent memory architectures discussed 2026-04-25 (native `AGENT.md` context engine, local SQLite/MCP, GCP Memory Bank), option 1 + sharing System OS paths preserves the Cognitive Sync Law without standing up parallel infrastructure. Option 3 (Memory Bank / Vertex AI) is forbidden by `project_no_gcp`. Option 2 (MCP via `mios-mcp.service` port 8051) remains a future fallback if `@`-imports prove flaky.
...
- `.ai/agent-state/settings.json` carries Agent-CLI runtime config and points at AGENT.md as `contextFileName`. `.ai/agent-state/.env` has `GOOGLE_CLOUD_PROJECT=""` and `OTLP_GOOGLE_CLOUD_PROJECT=""` to neutralise GCP telemetry.
