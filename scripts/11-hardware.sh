...
#
# CHANGELOG:
#   v2.3.2: Dropped COPY-layer fallback. ucore-hci IS already built from
...
#   v2.3.0: (attempted COPY-layer, reverted)
...
# exact failure mode that broke v2.2.x when ucore kernel (6.19.10) didn't
# match F44's kernel-devel (6.19.12). If kernel-devel is unavailable, we log
