#!/bin/bash
# CloudWS v0.1.4 — 99-cleanup: Final image cleanup (mirrors ucore/cleanup.sh)
#
...
# 4. ostree container commit — CRITICAL: finalizes OSTree layer metadata
echo "[99-cleanup] Running ostree container commit..."
ostree container commit 2>&1 || true

# 5. Recreate /var/tmp (required by systemd)
