#!/usr/bin/env python3
"""
validate-kargs.py — CloudWS-bootc kargs.d schema validator.

Checks every *.toml in:
  kargs.d/                              (repo root drop-ins)
  system_files/usr/lib/bootc/kargs.d/  (image-baked drop-ins)
...
Schema rules (bootc-dev/bootc authoritative):
  ...
  - Keys with "delete" in their name are hallucinated by Copilot — reject.
    ...
    for key in data:
        if key not in ALLOWED_TOP_KEYS:
            if "delete" in key.lower():
                msg = (
                    f"'{key}' is a Copilot hallucination — bootc has no delete mechanism "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate CloudWS-bootc kargs.d TOML files.")
