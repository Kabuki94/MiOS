...
echo -e "${YELLOW}[WARN] Pre-enrolled VARS file not found${NC}"
...
if command -v yay &>/dev/null; then
    ...
    echo -e "${YELLOW}[WARN] No AUR helper found (yay/paru)${NC}"
fi
...
case $choice in
    1)
        ...
        if command -v wget &>/dev/null; then
            wget -q --show-progress "$LATEST_URL" -O ovmf.rpm || {
                echo -e "${RED}[FAIL] Download failed${NC}"
                exit 1
            }
            ...
            echo -e "${RED}[FAIL] Neither wget nor curl available${NC}"
            exit 1
        fi
        ...
        if command -v rpm2cpio &>/dev/null; then
            ...
            echo -e "${RED}[FAIL] No extraction tool available (rpm2cpio or bsdtar)${NC}"
        ...
        if [ -z "$EXTRACTED_CODE" ] || [ -z "$EXTRACTED_VARS" ]; then
            echo -e "${RED}[FAIL] Could not find OVMF files in package${NC}"
            ls -R
            exit 1
        fi
    ...
    2)
        ...
        if [ ! -f "$TEMPLATE_VARS" ]; then
            echo -e "${RED}[FAIL] Template VARS file not found: $TEMPLATE_VARS${NC}"
            exit 1
        fi
