...
echo -e "     Search for: edk2-ovmf-secureboot, ovmf-git, etc."
echo
...
case $choice in
    1)
        ...
        if command -v rpm2cpio &>/dev/null && command -v cpio &>/dev/null; then
            ...
            echo -e "${YELLOW}Install: sudo pacman -S rpmextract${NC}"
            exit 1
        fi
    ...
    3)
        echo -e "${CYAN}Checking AUR for OVMF packages...${NC}\n"
        
        if command -v yay &>/dev/null; then
            yay -Ss ovmf edk2 secureboot
        elif command -v paru &>/dev/null; then
            paru -Ss ovmf edk2 secureboot
        else
            echo -e "${YELLOW}No AUR helper found${NC}"
            echo -e "Install yay or paru, then search:"
            echo -e "  ${CYAN}yay -Ss ovmf secureboot${NC}"
        fi
