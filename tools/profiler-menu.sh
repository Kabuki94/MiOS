...
show_banner() {
    ...
    cat << 'EOF'
*****************************************************************
*           INTERACTIVE SYSTEM PROFILER MENU                    *
*              Choose Your Tools                                *
*****************************************************************
EOF
    echo -e "${NC}\n"
...
show_menu() {
    ...
    if [ "$QUICK_SUMMARY_DONE" = true ]; then
        echo -e "  ${GREEN}${NC} 1) Quick System Summary        ${CYAN}[COMPLETED]${NC}"
    else
        echo -e "    1) Quick System Summary        ${YELLOW}[~30 seconds]${NC}"
    fi
    
    if [ "$IOMMU_DONE" = true ]; then
        echo -e "  ${GREEN}${NC} 2) IOMMU Group Analyzer        ${CYAN}[COMPLETED]${NC}"
    else
        echo -e "    2) IOMMU Group Analyzer        ${YELLOW}[~1-2 minutes]${NC}"
    fi
    
    if [ "$FULL_PROFILE_DONE" = true ]; then
        echo -e "  ${GREEN}${NC} 3) Full System Profiler        ${CYAN}[COMPLETED]${NC}"
    else
...
run_quick_summary() {
    ...
    if [ -f "$SCRIPT_DIR/quick-summary.sh" ]; then
        ...
        echo ""
        echo -e "${GREEN} Complete!${NC} Saved to: $OUTPUT_DIR"
    else
        echo -e "${RED} Error: quick-summary.sh not found!${NC}"
    fi
...
run_iommu_analyzer() {
    ...
    if [ -f "$SCRIPT_DIR/iommu-visualizer.sh" ]; then
        ...
        echo -e "${RED} Error: iommu-visualizer.sh not found!${NC}"
    fi
...
run_full_profiler() {
    ...
    if [ -f "$SCRIPT_DIR/system-profiler.sh" ]; then
        ...
        echo -e "${RED} Error: system-profiler.sh not found!${NC}"
    fi
...
run_all_tools() {
    ...
    if [ -f "$SCRIPT_DIR/run-all-profilers.sh" ]; then
        ...
        echo -e "${RED} Error: run-all-profilers.sh not found!${NC}"
        echo ""
        ...
        if [ "$first" -ge 1 ] && [ "$first" -le "${#profiles[@]}" ] && \
           [ "$second" -ge 1 ] && [ "$second" -le "${#profiles[@]}" ]; then
            ...
            if [ -f "$SCRIPT_DIR/profile-compare.sh" ]; then
                ...
                echo -e "${RED} Error: profile-compare.sh not found!${NC}"
            fi
        else
...
check_system_status() {
    ...
    if grep -q -E '(vmx|svm)' /proc/cpuinfo 2>/dev/null; then
        echo -e "  ${GREEN}${NC} CPU virtualization enabled"
    else
    ...
    if [ -d /sys/kernel/iommu_groups ]; then
        ...
        echo -e "  ${RED}${NC} IOMMU not available"
    fi
    ...
    if [ "$gpu_count" -gt 0 ]; then
        ...
        echo -e "  ${YELLOW}${NC} No discrete GPU detected"
    fi
