...
#
#  Cloud-WS Unified Management System
#  Version: 2.0.0 - Complete Integrated Edition
#
#  A single comprehensive tool for:
#    â€¢ Pre-installation system assessment
#    â€¢ Full Cloud-WS installation (15 phases)
...
#
#  Usage:
#    sudo ./cloudws-full.sh                    # Interactive menu
#    sudo ./cloudws-full.sh [mode] [options]   # Direct mode
#
#  Modes:
...
#==============================================================================
readonly CLOUDWS_VERSION="2.0.0"
readonly CLOUDWS_NAME="Cloud-WS Unified Management System"
readonly CLOUDWS_CODENAME="Integrated Edition"

# Directory structure
readonly CLOUDWS_BASE="/var/lib/cloudws"
readonly CLOUDWS_LOG_DIR="/var/log/cloudws"
readonly CLOUDWS_STATE_FILE="$CLOUDWS_BASE/state.json"
readonly CLOUDWS_CONFIG_DIR="$CLOUDWS_BASE/config"
readonly CLOUDWS_BACKUP_DIR="$CLOUDWS_BASE/backups"
...
#==============================================================================
# COLOR THEME - Cloud-WS Professional (Teal/Coral/White)
#==============================================================================
# Primary palette
...
init_logging() {
    mkdir -p "$CLOUDWS_LOG_DIR" 2>/dev/null || true
    mkdir -p "$CLOUDWS_BASE" 2>/dev/null || true
    mkdir -p "$CLOUDWS_CONFIG_DIR" 2>/dev/null || true
    mkdir -p "$CLOUDWS_BACKUP_DIR" 2>/dev/null || true
    
    LOG_FILE="$CLOUDWS_LOG_DIR/session-$SESSION_ID.log"
    DEBUG_LOG="$CLOUDWS_LOG_DIR/debug-$SESSION_ID.log"
    REPORT_FILE="$CLOUDWS_LOG_DIR/report-$SESSION_ID.txt"
    
    # Set ownership for user access
    if [[ $EUID -eq 0 && -n "${SUDO_USER:-}" ]]; then
        chown -R "$REAL_USER:$(id -gn "$REAL_USER")" "$CLOUDWS_LOG_DIR" 2>/dev/null || true
        chown -R "$REAL_USER:$(id -gn "$REAL_USER")" "$CLOUDWS_BASE" 2>/dev/null || true
    fi
    
    # Initialize log files
    {
        echo "================================================================================"
        echo " Cloud-WS Unified Management System - Session Log"
        echo " Version: $CLOUDWS_VERSION"
...
load_state() {
    if [[ -f "$CLOUDWS_STATE_FILE" ]]; then
        ...
        done < "$CLOUDWS_STATE_FILE"
        log "INFO" "Loaded state from $CLOUDWS_STATE_FILE"
    fi
}

save_state() {
    mkdir -p "$(dirname "$CLOUDWS_STATE_FILE")" 2>/dev/null
    
    cat > "$CLOUDWS_STATE_FILE" << EOF
{
    "version": "$CLOUDWS_VERSION",
...
EOF
    ...
    # Set ownership
    if [[ $EUID -eq 0 && -n "${SUDO_USER:-}" ]]; then
        chown "$REAL_USER:$(id -gn "$REAL_USER")" "$CLOUDWS_STATE_FILE" 2>/dev/null || true
    fi
    
    log "INFO" "Saved state to $CLOUDWS_STATE_FILE"
}
...
print_banner() {
    ...
    â•‘                                                                           â•‘
BANNER
    echo -e "    â•‘           ${WHITE}Unified Management System${C_HEADER}  v${CLOUDWS_VERSION}                            â•‘"
...
print_small_banner() {
    ...
    echo -e "${C_HEADER}    â”‚${NC}  ${WHITE}Cloud-WS${NC}  â”‚  ${C_SUBHEADER}$title${NC}"
...
POLKIT
    ...
    msg_success "Hostname set: Cloud-WS"
}
...
# Generated by Cloud-WS Unified on $(date)
...
generate_diagnostic_report() {
    ...
    local report="$REAL_HOME/cloudws-diagnostic-$TIMESTAMP.txt"
...
view_logs() {
    ...
    case "$choice" in
        1)
            if [[ -d "$CLOUDWS_LOG_DIR" ]]; then
                ls -lt "$CLOUDWS_LOG_DIR"/*.log 2>/dev/null | head -10
                echo ""
                read -rp "      View latest? [y/N]: " view
                if [[ "$view" =~ ^[Yy] ]]; then
                    local latest=$(ls -t "$CLOUDWS_LOG_DIR"/*.log 2>/dev/null | head -1)
                    [[ -f "$latest" ]] && less "$latest"
                fi
            else
                msg_info "No Cloud-WS logs yet"
            fi
            ;;
        2)
...
LOG_FILE="/var/log/cloudws/hook-$VM_NAME.log"
mkdir -p /var/log/cloudws
...
show_main_menu() {
    ...
    case "${STATE[install_phase]}" in
        completed)
            echo -e "${C_HEADER}    â•‘${NC}   ${C_SUCCESS}âœ”${NC} ${WHITE}2)${NC} Cloud-WS Installation          ${C_MUTED}[Completed]${NC}                  ${C_HEADER}â•‘${NC}"
            ;;
        in_progress)
            echo -e "${C_HEADER}    â•‘${NC}   ${C_WARNING}â—${NC} ${WHITE}2)${NC} Cloud-WS Installation          ${C_WARNING}[In Progress]${NC}                ${C_HEADER}â•‘${NC}"
            ;;
        *)
            echo -e "${C_HEADER}    â•‘${NC}     ${WHITE}2)${NC} Cloud-WS Installation          ${C_INFO}[~20 minutes]${NC}                ${C_HEADER}â•‘${NC}"
            ;;
    esac
...
menu_installation() {
    while true; do
        print_small_banner "Cloud-WS Installation"
...
run_full_profile() {
    ...
    local profile_file="$REAL_HOME/cloudws-profile-$TIMESTAMP.txt"
...
show_system_status() {
    ...
    print_subsection "Cloud-WS State"
...
show_help() {
    ...
    â€¢ Full Cloud-WS installation (virtualization host setup)
    ...
    sudo ./cloudws-full.sh                 Interactive menu
    sudo ./cloudws-full.sh assess          System assessment
    sudo ./cloudws-full.sh install         Run installation
    sudo ./cloudws-full.sh vfio            Configure VFIO
    ...
    sudo ./cloudws-full.sh status          Show status
    sudo ./cloudws-full.sh help            Show this help

    TYPICAL WORKFLOW
    â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    1. Run Assessment â†’ Check hardware compatibility
    2. Run Installation â†’ Install Cloud-WS
    ...
    FILES
    â”€â”€â”€â”€â”€
    Logs:   /var/log/cloudws/
    State:  /var/lib/cloudws/state.json
    Hooks:  /etc/libvirt/hooks/

    For more information, visit:
    https://github.com/your-repo/cloud-ws
...
view_session_logs() {
    ...
    if [[ -d "$CLOUDWS_LOG_DIR" ]]; then
        ...
        ls -lt "$CLOUDWS_LOG_DIR"/*.log 2>/dev/null | head -10 | while read -r line; do
            echo "      $line"
        done
        ...
        case "$choice" in
            ...
            2)
                ...
                ;;
            3)
                local archive="$REAL_HOME/cloudws-logs-$TIMESTAMP.tar.gz"
                tar -czf "$archive" -C "$CLOUDWS_LOG_DIR" . 2>/dev/null
    ...
    while true; do
        ...
        case "$choice" in
            ...
            q|Q)
                ...
                save_state
                echo ""
                echo -e "    ${C_SUCCESS}Thank you for using Cloud-WS!${NC}"
                echo ""
                exit 0
                ;;
            *)
...
handle_cli_args() {
    case "${1:-}" in
        ...
        help|-h|--help)
            ...
            ;;
        version|-v|--version)
            echo "Cloud-WS Unified Management System v$CLOUDWS_VERSION"
            echo "$CLOUDWS_CODENAME"
            ;;
        "")
            # Interactive mode
            init_logging
        ...
        *)
            ...
            echo "  install     Cloud-WS installation"
