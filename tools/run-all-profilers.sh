...
print_banner() {
    ...
    cat << 'EOF'
*****************************************************************
*              SYSTEM PROFILER - CHAIN RUNNER                   *
*           Running All Tools Consecutively                     *
*****************************************************************
EOF
    echo -e "${NC}\n"
}
...
generate_summary() {
    ...
    
    {
        ...
        if grep -q "System ready for MiOS-Build" "$RUN_DIR/01-quick-summary.txt" 2>/dev/null; then
            echo " VERDICT: System appears ready for MiOS-Build"
        else
...
show_results() {
    ...
    echo -e "${BOLD}${GREEN} All tools executed successfully!${NC}\n"
