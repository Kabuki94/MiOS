...
for location in "${OVMF_LOCATIONS[@]}"; do
    if [ -f "$location/OVMF_CODE.secboot.fd" ]; then
        ...
        OVMF_VARS="$location/OVMF_VARS.fd"
        echo -e "${YELLOW}[WARN] Found standard OVMF (no Secure Boot) at: $location${NC}"
        break
    fi
done

if [ -z "$OVMF_CODE" ]; then
    echo -e "${RED}[FAIL] No OVMF firmware found!${NC}"
    ...
    if [ -z "$OVMF_CODE" ]; then
        echo -e "${RED}[FAIL] Still can't find OVMF files after installation!${NC}"
...
if [ ! -f "$OVMF_VARS" ]; then
    echo -e "${RED}[FAIL] OVMF_VARS template doesn't exist: $OVMF_VARS${NC}"
...
if virsh dominfo Xbox &>/dev/null; then
    ...
    echo -e "${YELLOW}[WARN] VM 'Xbox' not currently defined${NC}"
fi
...
if [ -f "$NVRAM_PATH" ]; then
    ...
    echo -e "${YELLOW}[WARN] No existing NVRAM found (first boot)${NC}"
fi
...
if virsh define /tmp/Xbox-fixed.xml; then
    ...
    echo -e "${RED}[FAIL] Failed to apply configuration${NC}"
...
if virsh start Xbox; then
    ...
    echo -e "${RED}[FAIL] Failed to start VM${NC}"
