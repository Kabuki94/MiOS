...
echo ">>> Installing @google.ai/agent-state-cli..."
${SUDO} npm install -g @google.ai/agent-state-cli@latest

echo ">>> Preparing IDE Extension Inter-Process Communication context..."
cat << 'EOF_BASH' >> "$HOME/.bashrc"
if [ ! -f "$HOME/.ai/agent-state/.ide_setup_complete" ]; then
    echo "================================================================="
    echo "Agent CLI Dev Container Initialization Protocol"
    echo "Target Infrastructure Project: mios-os"
    echo "================================================================="
    echo "To finalize native VSCodium integration, please run:"
    echo "1..ai/agent-state"
    echo "2. /ide install"
    echo "3. /ide enable"
    echo "================================================================="
    mkdir -p "$HOME/.ai/agent-state"
    touch "$HOME/.ai/agent-state/.ide_setup_complete"
fi
EOF_BASH
