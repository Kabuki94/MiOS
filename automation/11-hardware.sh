...
#
# CHANGELOG:
#   v0.1.3: Dropped COPY-layer fallback. ucore-hci IS already built from
...
#   v0.1.3: (attempted COPY-layer, reverted)
...
# exact failure mode that broke v2.2.x when ucore kernel (v0.1.3) didn't
# match F44's kernel-devel (v0.1.3). If kernel-devel is unavailable, we log
