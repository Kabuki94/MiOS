#!/bin/bash
# MiOS v2.1.0 — 11-hardware: GPU drivers (Mesa + AMD ROCm + Intel + NVIDIA)
#
# NVIDIA strategy (v2.1.0):
...
#
# CHANGELOG:
#   v2.1.0: Dropped COPY-layer fallback. ucore-hci IS already built from
...
#   v2.1.0: (attempted COPY-layer, reverted)
...
# exact failure mode that broke v2.2.x when ucore kernel (v2.1.0) didn't
# match F44's kernel-devel (v2.1.0). If kernel-devel is unavailable, we log
