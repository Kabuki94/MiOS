...
> **Note:** This agent was previously named .ai/agent-state-fullstack` and has been renamed to `deep-search`. If you're looking for the old .ai/agent-state-fullstack` agent, you're in the right place! All functionality remains the same.

The **Deep Search Agent Development Kit (ADK) Quickstart** is a production-ready blueprint for building a sophisticated, fullstack research agent with Agent. It's built to demonstrate how the ADK helps structure complex agentic workflows, build modular agents, and incorporate critical Human-in-the-Loop (HITL) steps.
...
<img src="https://github.com/CloudCloudPlatform/agent-starter-pack/blob/main/specs/images/adk.ai/agent-state_fullstack.gif?raw=true" width="80%" alt="Agent Fullstack ADK Preview">

This project adapts concepts from the [Agent FullStack LangGraph Quickstart](https://github.com/google.ai/agent-state.ai/agent-state-fullstack-langgraph-quickstart) for the frontend app.
...
-   [uv](https://github.com/astral-sh/uv) for dependency management
    ...
    ```
-   [Cloud Cloud SDK](https://cloud.google.com/sdk/specs/install) and a **Cloud Cloud Project** with the **Vertex AI API** enabled

## Agent Starter Pack (recommended)

Use the [Agent Starter Pack](https://goo.gle/agent-starter-pack) to scaffold a production-ready project and choose your deployment target ([Vertex AI Agent Engine](https://cloud.google.com/vertex-ai/generative-ai/specs/agent-engine/overview) or [Cloud Run](https://cloud.google.com/run)), with CI/CD and other production features. The easiest way is with [uv](https://docs.astral.sh/uv/) (one command, no venv or pip install needed):
...
The starter pack will prompt you to select deployment options and set up your Cloud Cloud project.
...
You'll need a **[Cloud AI Studio API Key](https://aistudio.google.com/app/apikey)**.
...
Make sure you're authenticated with Cloud Cloud:
```bash
...
Once deployed, grant users access to your IAP-protected service by following the [Manage User Access](https://cloud.google.com/run/specs/securing/identity-aware-proxy-cloud-run#manage_user_or_group_access) documentation.
...
![ADK Agent Fullstack Architecture](https://github.com/CloudCloudPlatform/agent-starter-pack/blob/main/specs/images/adk.ai/agent-state_fullstack_architecture.png?raw=true)
...
You can edit key parameters (Agent models, research loop iterations) in the `ResearchConfiguration` dataclass within `app/config.py`.
...
*   **Adjusting Research Parameters:** Key parameters, such as the Agent models used or the number of research loop iterations, can be adjusted in the `ResearchConfiguration` dataclass within `app/config.py`.
...
> **User:** A report on the last Cloud I/O
>
> **Agent:** Here's a research plan for a report on the last Cloud I/O:
>
> *   Analyze the key announcements and product updates made at Cloud I/O 2025.
> *   Identify the central themes and strategic directions emphasized by Cloud during the event, particularly concerning AI, Android, and other emerging technologies like Android XR.
> *   Investigate the reception and industry impact of the major product unveilings and feature enhancements.
> *   Examine any significant shifts or continuities in Cloud's developer strategy as presented at the conference.
...
> **Agent:**
> 
> Cloud I/O 2025: Key Announcements and Highlights
> ....

## Troubleshooting

If you encounter issues while setting up or running this agent, here are some resources to help you troubleshoot:
- [ADK Documentation](https://google.github.io/adk-specs/): Comprehensive documentation for the Agent Development Kit
- [Vertex AI Authentication Guide](https://cloud.google.com/vertex-ai/specs/authentication): Detailed instructions for setting up authentication
...
*   [**Cloud Agent**](https://cloud.google.com/vertex-ai/generative-ai/docs): Used for planning, reasoning, search query generation, and final synthesis.
