...
| `/etc` | **3-way merge** on upgrades (baselocalnew) | Set `etc.transient = true` for fully ephemeral |
...
4. Missing `tmpfiles.d` entries for `/var` directories ([WARN], not error, as of v0.1.3)
...
| **Logically bound** | Separate OCI image; declared in `/usr/lib/bootc/bound-images.d/` | [OK] Yes  on `bootc upgrade`/`install` | Only downloaded when app image changes |
...
Per bootc issue #1190 (composefs-native backend umbrella), the composefs-native backend is **still under active development** and has not achieved feature parity with the ostree backend. Work in progress (as of FebApr 2026):
...
- Digest pins in `image-versions.yml` may see more frequent churn during the transition window (AprilJune 2026)
...
|--------|------------------|-------|
| `ext4` | [OK] Yes | Recommended for MiOS; fsverity supported kernel  5.4 |
| `btrfs` | [OK] Yes | Alternative; fsverity supported kernel  5.15 |
| `xfs` | [FAIL] No | fsverity NOT supported; `composefs.enabled = verity` will error at install |
...
**Notable architectural changes (20252026):**
...
- **4K/240Hz compositor regression (GSP firmware, KWin/GNOME Mutter):** Reported desktop-compositor stutter at 25601440@240Hz under KWin with GSP firmware; risk is likely higher at 4K/240Hz on RTX 4090. **Validate on 9950X3D+RTX 4090 before defaulting to open modules in MiOS.** Gate with `34-gpu-detect.sh` and allow proprietary escape-hatch.
...
- `fapolicyd` is available but historically caused 25 minute boot delays when hashing all binaries; if used, configure the RPM database trust backend exclusively to avoid hashing overhead
...
*Research thread: Fedora bootc official images, FCOSOCI roadmap, Rawhide status*
...
| ext4 |  5.4 | [OK] Yes |
| btrfs |  5.15 | [OK] Yes |
| XFS | Not supported | [FAIL] No  bootc errors at install |
| F2FS |  5.4 | [OK] (not tested with bootc) |
| tmpfs | Not supported | [FAIL] No |
...
**Quadlet warns on known-bad keys:** When generated units include `User=`, `Group=`, or `DynamicUser=` in `[Service]`, Quadlet now emits a [WARN]  those options are known-incompatible with Podman's security model.
...
1. After F44 rebase, audit any `.container` files for new lint [WARN]s (5.8 added stricter parsing of `User=`/`Group=`/`DynamicUser=`).
...
<!-- FINDINGS BEGIN -->
### nvidia-container-toolkit release history (20242026)
...
**Confirmed changes (Renovate v36v38, 2024mid-2025):**
...
- **`dockerfile` manager gains Containerfile support (v37v38):** Bootc projects no longer need `fileMatch` overrides to detect root-level `Containerfile`  it is auto-detected alongside `Dockerfile`.
...
|-------|-----------|--------|
| Image signing | cosign keyless (Fulcio/Rekor) | [OK] Operational (`build-sign.yml`) |
| SBOM | syft SPDX-JSON + CycloneDX | [OK] Fixed glob (`build.yml` April 2026) |
| Policy enforcement | containers/policy.json + ublue cosign pub key | [OK] Shipped in system_files |
| Composefs verity | ext4 + fsverity per-file verification | [OK] Active (`40-composefs-verity.sh`) |
| SELinux enforcing | Fedora default; composefs-compatible | [OK] Active |
| USBGuard | usbguard-daemon.conf 0600 | [OK] Fixed (April 2026) |
| fapolicyd | RPM trust + custom trust for /usr/libexec/mios | [WARN] Verify trust DB after exec bit fix |
| CrowdSec | Sovereign IDS + nftables bouncer | [OK] Enabled (WSL2 gated) |
| MOK / Secure Boot | Universal Blue akmods-ublue.der | [OK] Enrolled via enroll-mok.sh |

### 2026-04-25 update  CVE-2026-4631 (Cockpit unauthenticated RCE, CVSS 9.8) [WARN]
...
- **Affected vector requires:** Cockpit 327359 + OpenSSH **older than 9.6**.
