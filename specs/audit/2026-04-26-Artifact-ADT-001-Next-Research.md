...
2. **Watch ucore-hci digest**  poll `{{MIOS_BASE_IMAGE}}` for F44 tag (2448h post-GA)
...
- **Issue:** Script exits `0` on K3s failure  greenboot cannot log the [WARN]
- **Fix:** Change failure-path `exit 0` to `exit 1` (wanted.d logs non-zero as [WARN], does NOT trigger rollback)
...
- Confirm `45-nvidia-cdi-refresh.sh` 1.18 requirement satisfied (v1.19.0 > 1.18 [OK])
...
- **bootc issues #21302132:** https://github.com/bootc-dev/bootc/issues
