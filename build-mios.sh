...
collect_user_config() {
    ...
    while true; do
        ...
        if [[ "$MIOS_PASSWORD" == "$MIOS_PASSWORD_CONFIRM" ]]; then
            ...
            MIOS_PASSWORD_HASH=$(python3 -c "import crypt; print(crypt.crypt('${MIOS_PASSWORD}', crypt.mksalt(crypt.METHOD_SHA512)))")
            break
        else
