#!/bin/bash
# CloudWS greenboot — required network reachability check
...
echo "[cloudws-greenboot] Checking DNS reachability of ${REGISTRY_HOST}..."

# Wait up to TIMEOUT seconds for DNS to become available
deadline=$(( $(date +%s) + TIMEOUT ))
while true; do
    if systemd-resolve "${REGISTRY_HOST}" >/dev/null 2>&1; then
        echo "[cloudws-greenboot] DNS OK: ${REGISTRY_HOST} resolved successfully."
        exit 0
    fi
    if [[ $(date +%s) -ge $deadline ]]; then
        echo "[cloudws-greenboot] FAIL: DNS resolution of ${REGISTRY_HOST} timed out after ${TIMEOUT}s." >&2
        exit 1
    fi
    sleep 2
done
