# CloudWS-bootc Upstream Research Plan
...
> ecosystem survey and maps each finding to an actionable CloudWS item.

---

## 1. bootc Upstream (bootc-dev/bootc v1.15.1)

### 1.1 New Commands — Implement via MOTD / tooling
| Finding | CloudWS Action |
|---------|---------------|
...
| `bootc usroverlay --readonly` (v1.15.0) | Document in DIAGNOSTICS.md as inspection tool |
...
| Finding | CloudWS Action |
|---------|---------------|
| ucore-hci now on NVIDIA 595.58.03 open modules | Inherited via base image — no action |
...
| CDI is default mode in nvidia-container-toolkit v1.19.0 | Already enabled via `nvidia-cdi-refresh.path/.service` — correct |
| DO NOT use nvidia-container-toolkit v1.17.8 (CDI regression) | Tracked in research notes |
...
| `cockpit.socket` race with `libvirtd.socket` | Add `After=libvirtd.socket` to `cockpit.socket.d/10-cloudws.conf` |
| `libvirtd` 45s shutdown timeout too short | Already fixed in `libvirtd.service.d/10-cloudws.conf` (TimeoutStopSec=120) |
| `uBlue COPR repos disabled before generic installs` (Apr 2026 commit) | If CloudWS re-enables COPR repos, wrap with explicit enable/disable |
| `ublue-os/cayo` — composefs-native HCI successor to ucore-hci | Monitor for CloudWS-3 base migration (no action now) |

### 2.4 Cosign / Signing (CRITICAL — DO NOT UPGRADE)
| Finding | CloudWS Action |
|---------|---------------|
| Cosign v3 `--new-bundle-format` BREAKS rpm-ostree/bootc (rpm-ostree#5509) | Stay on cosign v2.6.3 — already pinned correctly |
| `cosign-installer@v3.10.1` with `cosign-release: v2.6.3` — correct pattern | Already correct in `build.yml` — no change |
...
| Finding | CloudWS Action |
|---------|---------------|
| `net.core.bpf_jit_harden = 2` (Fedora 44 default) | Add to `99-cloudws-hardening.conf` now (ahead of F44) |
| `kernel.yama.ptrace_scope = 1` (Fedora 44 default) | Already at `= 2` — more restrictive, correct |
| `kernel.unprivileged_bpf_disabled = 1` | Add to `99-cloudws-hardening.conf` |
| `kernel.sysrq = 0` (production hardening) | Add to `99-cloudws-hardening.conf` |
| `kernel.printk = 3 3 3 3` (suppress kernel log to console) | Add to `99-cloudws-hardening.conf` |
...
| Finding | CloudWS Action |
|---------|---------------|
| `HttpProxy=false` in `[Container]` — prevents host proxy credential leak to containers | Add to ALL CloudWS Quadlet `.container` files |
...
| Airgap `.cache.json` for conditional image import (v1.33.1+) | Add to k3s-manifests if airgap deployment is needed |
...
| Finding | CloudWS Action |
|---------|---------------|
| `greenboot.conf` with `GREENBOOT_MAX_BOOT_ATTEMPTS=3`, `GREENBOOT_WATCHDOG_CHECK_ENABLED=true` | Create `system_files/etc/greenboot/greenboot.conf` (currently missing) |
| `greenboot-rs` v0.16.0+ — same script directories, same systemd integration | No structural changes needed |
...
| Finding | CloudWS Action |
|---------|---------------|
| `spectre_bhi=on` — Branch History Injection mitigation (newer Spectre variant) | Add to `01-cloudws-hardening.toml` |
| `kvm.nx_huge_pages=force` — KVM security for NX huge pages | Add to `01-cloudws-hardening.toml` |
| `tsx=off` — Transactional Sync Extensions (Intel security) | Add to `01-cloudws-hardening.toml` |
...
| Finding | CloudWS Action |
|---------|---------------|
| `ublue-os/bootc-image-builder-action` is in maintenance mode; upstream is `osbuild/bootc-image-builder-action@v0.0.2` | Evaluate migration — note in research doc; do not rush |
...
| Finding | CloudWS Action |
|---------|---------------|
| cockpit.socket may start before libvirtd.socket, breaking Machines UI | Add `cockpit.socket.d/10-cloudws.conf` with `[Unit] After=libvirtd.socket` |
...
- Cosign v2.6.3 pinned, `--new-bundle-format=false` in all signing steps ✅
- `crowdsec-dashboard.container`, `cloudws-guacamole.container`, `guacamole-postgres.container`, `guacd.container` all have `GlobalArgs=--storage-opt=additionalimagestore=/usr/lib/bootc/storage` ✅
- `libvirtd.service.d/10-cloudws.conf` has `After=libvirtd.socket` + `TimeoutStopSec=120` ✅
