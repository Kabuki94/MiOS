# CloudWS-bootc v2.1: resolving every build and boot failure
...
RUN --mount=type=bind,from=ctx,source=/,target=/ctx \
    cp -r /ctx/system_files/* / && /ctx/scripts/10-gnome.sh
```
...
- **image-builder CLI** (from the `@osbuild/image-builder` COPR) is installable as an RPM and is expected to merge with BIB eventually. It supports `--force-data-dir` for custom repo configurations but currently focuses on traditional package-based inputs rather than bootc containers.
