...
| 6 | **Podman-machine backend compatibility shim** | `containers/podman-machine-os` + `containers/podman-machine-wsl-os` (now merged) | `podman machine init --image ghcr.io/kabuki94/cloudws-bootc:latest` becomes your Windows dev loop |
...
| 10 | **BlueBuild recipe.yml for multi-variant** (CloudWS-1, CloudWS-2) | `github.com/blue-build` | Matrix builds from one repo with modular recipe stages |
...
`podman machine os apply <image> <machine>` runs `bootc switch` inside the guest. Supported transports: `docker://`, `oci-archive:`, `containers-storage:`. That means **any bootc image (including CloudWS-bootc) can be applied today with `podman machine os apply ghcr.io/kabuki94/cloudws-bootc:latest podman-machine-default`** as long as the image satisfies the machine contract below. Note WSL-based machines are documented as *not* upgradable via `os apply` — users fall back to `podman machine ssh` + dnf; however since WSL machine-os is now a bootc image, `bootc switch` should work, just unsupported officially.

### Minimal Containerfile stanza for Podman-machine compatibility

```dockerfile
# CloudWS-bootc Podman-machine compatibility layer
RUN dnf -y install \
...
Ship this as `/usr/lib/systemd/system-preset/91-cloudws-nvidia.preset`:
...
Ship `/usr/lib/systemd/system/cloudws-cdi-detect.service`:

```ini
[Unit]
Description=CloudWS CDI spec detection and selection
Before=nvidia-cdi-refresh.service
ConditionPathExists=!/var/lib/cloudws/cdi-selected

[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/libexec/cloudws/select-cdi-spec
[Install]
WantedBy=multi-user.target
```

`/usr/libexec/cloudws/select-cdi-spec` is a tiny shell script that branches on `/dev/dxg` presence: if present, it invokes `nvidia-ctk cdi generate --mode=wsl --output=/var/run/cdi/nvidia.yaml`; otherwise the default `--mode=auto`. This mirrors what `podman-machine-os` does implicitly via toolkit's own detection, but an explicit unit makes debugging tractable.
...
[boot]
systemd=true
command="/usr/libexec/cloudws-wsl-firstboot"
...
- `system_files/shared/` tree maps 1:1 to `/`. Drop in systemd presets under `system_files/shared/usr/lib/systemd/system-preset/`.
...
- DX variant split: a second Containerfile stage layers `docker-ce`, `lxd`, `libvirt`, `incus` on top of base. Direct template for your CloudWS-1/CloudWS-2 split.
...
- `system_files/desktop/shared/usr/share/wayland-sessions/gamescope-session-steam.desktop`: the session file that launches `gamescope-session-plus steam`.
- `system_files/desktop/shared/usr/lib/systemd/system/bazzite-libvirtd-setup.service`: pattern for first-boot libvirt enablement (ConditionPathExists, restorecon, state-file). Clone verbatim for CloudWS libvirt setup.
- `system_files/desktop/shared/usr/share/ublue-os/just/82-bazzite-waydroid.just`: `ujust setup-waydroid` recipe — drives `sudo waydroid init` + SELinux context restore. Adopt the recipe layout; Waydroid itself is risky on pure-NVIDIA (see open questions).
...
- Build scripts sign via `/usr/src/kernels/$KERNEL/scripts/sign-file sha256 <priv> <pub> <module.ko>`.
...
- **For CloudWS**: do not run your own signing infrastructure. COPY from ublue-os/akmods* and inherit their MOK. Document MOK enrollment in your README using the same `akmods-ublue.der` path.
...
- `docs/example.butane`: Ignition config that downloads `install_secureblue.sh` with sha256 check, disables `zincati` + `rpm-ostreed-automatic`, then runs installer. Good template for your Anaconda kickstart.
...
- systemd preset model: `20-ignition.preset` enables `ignition-firstboot-complete.target`. Mirror the preset-driven approach for CloudWS's first-boot stack.
...
- Nebraska/Omaha update protocol is Flatcar-specific; **don't adopt**. Stay on bootc for CloudWS.
- **Key takeaway**: sysext on Fedora Rawhide bootc is technically feasible but under-tested. Skip for CloudWS 1.0; revisit when shipping optional GPU/K3s/Ceph modules.
...
- FIPS mode: set `fips=1` in `/usr/lib/bootc/kargs.d/15-fips.toml`, ship `/etc/system-fips` marker file, include `crypto-policies` package, run `fips-mode-setup --enable` at build time. **Warning**: FIPS mode breaks proprietary NVIDIA drivers because NVIDIA userspace uses non-FIPS OpenSSL. **Do not ship FIPS as default on a NVIDIA workstation image.** Offer as an optional variant (CloudWS-fips) only if required.
- Kickstart `ostreecontainer` directive is how Anaconda installs bootc images. Use for your installable ISO variant.

### systemd-sysext / confext / homed / sysupdate verdicts for CloudWS

- **sysext**: promising but defer. Ship CloudWS 1.0 as monolithic bootc image; consider extracting Gamescope + K3s as sysexts post-1.0 once Flatcar patterns are more broadly tested on Fedora bootc.
...
- **sysupdate**: complements but does not compete with bootc. Some discussion in bootc issues about using sysupdate for the ESP + bootloader (bootupd is today's answer). **Skip for CloudWS 1.0.**

## Composefs and fs-verity adoption roadmap

Fedora bootc base images **already enable composefs by default** in "unsigned" mode (fsverity on when the target filesystem supports it, no signature enforcement). The adoption arc for CloudWS is three phases.
...
This hard-requires fsverity at install time — BIB will refuse to install if the target filesystem doesn't support it (ext4 and btrfs do; xfs does not as of 2025-2026 — **use ext4 or btrfs for CloudWS, NOT xfs**). **Caveat**: `/etc` and `/var` mounts are unaffected; since `/etc/systemd/system/*.service` is executable config, an attacker with `/etc` write can still run code. Mitigate with `etc.transient = yes` if your threat model requires it — but this breaks many workstation workflows (persistent user SSH configs, NetworkManager keyfiles, etc.). Recommend: **do not** enable transient /etc for a workstation image; accept the /etc trust assumption.

**Phase 3 (requires upstream work)**: Full boot-chain integrity — signed composefs metadata + Unified Kernel Images (UKI). Tracked in `gitlab.com/fedora/bootc/tracker/-/issues/14`. Not ready for CloudWS 1.0; watch for bootupd + UKI support to land.
...
jobs:
  build:
    ...
    steps:
      ...
      - run: cosign sign --yes ghcr.io/kabuki94/cloudws-bootc@${{ steps.push.outputs.digest }}
      - uses: actions/attest-build-provenance@v2
        with:
          subject-name: ghcr.io/kabuki94/cloudws-bootc
          subject-digest: ${{ steps.push.outputs.digest }}
          push-to-registry: true
```

### Consumer policy.json for CloudWS users
...
Ship both files in the image. Users then invoke `bootc switch --enforce-container-sigpolicy ghcr.io/kabuki94/cloudws-bootc:latest` and pulls are verified. For SLSA, extend the `ujust update-system` recipe to also run `gh attestation verify <ghcr-image> --repo Kabuki94/CloudWS-bootc` before the switch.

### Generate your cosign keyless identity reference

Record in README that the expected signer identity is `https://github.com/Kabuki94/CloudWS-bootc/.github/workflows/build-sign.yml@refs/heads/main` with issuer `https://token.actions.githubusercontent.com`. Users verify offline via `cosign verify --certificate-identity <...> --certificate-oidc-issuer <...> ghcr.io/kabuki94/cloudws-bootc:latest`.
...
### Recommended updater stack for CloudWS
...
Greenboot-rs (Fedora 43 Change, Rust rewrite of greenboot) integrates natively with bootc. Drop your checks in `/etc/greenboot/check/required.d/` (fail → rollback) and `/etc/greenboot/check/wanted.d/` (fail → warn). Example for CloudWS:
...
### Per-target BIB invocations for CloudWS
...
podman create --name tmp-wsl ghcr.io/kabuki94/cloudws-bootc:latest /bin/true
podman export tmp-wsl | zstd -T0 -o output/cloudws-wsl.tar.zst
```
Users import via `wsl --import CloudWS C:\WSL\CloudWS cloudws-wsl.tar.zst --version 2`.
...
- Multi-arch builds: bootc container arch must match BIB arch. For aarch64 CloudWS variants, use an aarch64 BIB runner or build on a Graviton GitHub Actions runner.
...
**`/usr/lib/systemd/system/cloudws-cdi-detect.service`** — WSL vs bare-metal CDI selection oneshot (shown above).

**`/usr/lib/systemd/system/cloudws-libvirtd-setup.service`** — first-boot libvirt restorecon + socket enable (mirror of Bazzite's pattern).

**`/usr/lib/systemd/system/cloudws-crowdsec-init.service`** — first-boot bouncer key generation (shown in research notes; generates API key via `cscli bouncers add`, writes to bouncer yaml, restarts bouncer).

**`/usr/lib/systemd/system/cloudws-firstboot.target`** — aggregator target that pulls in all first-boot oneshots with `ConditionFirstBoot=yes`:

```ini
[Unit]
Description=CloudWS first-boot provisioning target
After=multi-user.target
Requires=cloudws-cdi-detect.service cloudws-libvirtd-setup.service cloudws-crowdsec-init.service
```

**`/usr/lib/systemd/system-preset/90-cloudws.preset`**:
...
enable cloudws-cdi-detect.service
enable cloudws-libvirtd-setup.service
enable cloudws-crowdsec-init.service
...
### Multi-variant structure (CloudWS-1 workstation + CloudWS-2 server)
...
# ----- CloudWS-1: workstation (GNOME, Gamescope, Waydroid, Looking Glass) -----
FROM base AS cloudws-1
...
# ----- CloudWS-2: HA node (K3s, Ceph, Pacemaker, no desktop) -----
FROM base AS cloudws-2
...
strategy:
  matrix:
    variant: [cloudws-1, cloudws-2]
    include:
      - variant: cloudws-1
        dockerfile-target: cloudws-1
      - variant: cloudws-2
        dockerfile-target: cloudws-2
```
...
```dockerfile
COPY system_files/etc/pki/containers/ /etc/pki/containers/
COPY system_files/etc/containers/policy.json /etc/containers/policy.json
COPY system_files/etc/containers/registries.d/ /etc/containers/registries.d/
```

## Open questions and gaps — what CloudWS will need to pioneer

These are the areas where upstream has no finished pattern; your work here will be novel and worth upstreaming back.

**bootc + K3s declarative bootstrap**: ucore ships k3s packages but no declarative bootstrap (no "install image + reboot = working single-node cluster"). You will have to write a first-boot oneshot that runs `k3s server --write-kubeconfig=/etc/rancher/k3s/k3s.yaml` with configuration from `/etc/cloudws/k3s-config.yaml` (machine-local state) — model after Flatcar's `kubelet.service` sysext pattern.
...
- bootc-image-builder: `github.com/osbuild/bootc-image-builder` and `osbuild.org/docs/bootc/`
- bootc-image-builder-action: `github.com/osbuild/bootc-image-builder-action`
- Fedora bootc docs (local preview, authoritative): `fedora.gitlab.io/bootc/docs/bootc/`
...
- Bazzite: `github.com/ublue-os/bazzite` (`system_files/desktop/shared/usr/lib/systemd/system/bazzite-libvirtd-setup.service`, `system_files/desktop/shared/usr/share/ublue-os/just/82-bazzite-waydroid.just`)
