...
**Hypothesis confirmed.** On ucore-hci / Fedora-CoreOS-lineage bootc images, `/usr/local` is a symbolic link to `/var/usrlocal` (the long-standing OSTree convention: `/usr` is immutable, `/var` is runtime-mutable, so a writable `/usr/local` must be a symlink into `/var`). bootc upstream has since diverged and recommends regular-directory `/usr/local` for **base** images, but derived images in the ublue/ucore stack still ship the symlink. `cp -a /ctx/system_files/usr/local/. /usr/local/` fails with `cannot create directory '/usr/local/': File exists` because the trailing slash forces `cp` to treat the target as a directory name to create — which collides with the existing symlink target.
...
RUN set -euo pipefail; \
    ...
    if [ -d /ctx/system_files/usr/local ]; then \
        ...
        tar -C /ctx/system_files/usr/local -cf - . \
...
**New file `scripts/lib/common.sh`** (create if absent; merge additively if present):
...
**Modification to `scripts/build.sh`:** keep the existing export so backward compatibility is preserved; add `source "$(dirname "$0")/lib/common.sh"` at the top after `set -euo pipefail`, and remove any line that re-assigns `DNF_SETOPT` as a string.
...
|---|---|
| `scripts/01-repos.sh` | RPM Fusion free/nonfree release RPM install URLs |
| `scripts/05-enable-external-repos.sh` | full replacement — see §3 |
| `scripts/10-core-packages.sh` (or wherever PACKAGES.md is consumed) | main install loop |
| `scripts/11-nvidia.sh` (if present) | akmod-nvidia, nvidia-driver-cuda |
| `scripts/12-container-tools.sh` (if present) | docker-ce family |
| `scripts/13-dev-tools.sh` (if present) | code, gh, tailscale, 1password |
| `scripts/14-virtualization.sh` (if present) | libvirt, qemu-kvm |
| `scripts/15-desktop-apps.sh` (if present) | google-chrome, fonts |
| `scripts/16-swaps.sh` (if present) | ffmpeg/mesa-freeworld swaps |
| `scripts/17-cleanup.sh` (or equivalent) | dnf remove of firefox/gnome-software-rpm-ostree/podman-docker |
| `scripts/lib/packages.sh` | the dnf-call wrapper around PACKAGES.md entries |

**Auditing command** to verify 100% coverage after the edit (run in CI as a shellcheck-adjacent gate):

```bash
! grep -rnE '^\s*dnf (install|remove|swap|group)' scripts/ \
  | grep -v '"${DNF_SETOPT\[@\]}"'
```
...
## 3. Full replacement `scripts/05-enable-external-repos.sh`

Design: no RPM Fusion (01 handles it), `dnf` not `dnf5`, `${DNF_SETOPT[@]}` on every mutating call, idempotent per-repo guard, fails fast. Includes Terra, VS Code, 1Password, Tailscale, Docker CE, Google Chrome — the six repos defensible for a developer workstation bootc. COPRs and aggressive privacy browsers (Brave, Mullvad, etc.) are deferred to Flatpak per ublue best-practice.
...
### 5b. `/etc/containers/policy.json` (shipped in-image via `system_files/`)
...
### 5d. `scripts/37-cosign-policy.sh`

```bash
#!/usr/bin/env bash
# scripts/37-cosign-policy.sh
...
#   system_files/sigstore/fulcio_v1.crt.pem   (from sigstore/root-signing)
#   system_files/sigstore/rekor.pub           (from sigstore/root-signing)
#   system_files/sigstore/ublue-os.pub        (from ublue-os/main cosign.pub)
...
install -m 0644 /ctx/system_files/sigstore/fulcio_v1.crt.pem \
    /etc/pki/containers/fulcio_v1.crt.pem
install -m 0644 /ctx/system_files/sigstore/rekor.pub \
    /etc/pki/containers/rekor.pub
install -m 0644 /ctx/system_files/sigstore/ublue-os.pub \
    /etc/pki/containers/ublue-os.pub

install -d -m 0755 /etc/containers
install -m 0644 /ctx/system_files/etc/containers/policy.json \
    /etc/containers/policy.json

install -d -m 0755 /etc/containers/registries.d
install -m 0644 /ctx/system_files/etc/containers/registries.d/ghcr.io-kabuki94.yaml \
...
| `scripts/lib/common.sh` (new) | `scripts__lib__common.sh` | DNF_SETOPT array + logging |
| `scripts/build.sh` (edit: source common.sh, drop string export) | `scripts__build.sh` | §2 |
| `scripts/01-repos.sh` (edit: adopt `${DNF_SETOPT[@]}`) | `scripts__01-repos.sh` | §2 |
| `scripts/05-enable-external-repos.sh` (full replacement) | `scripts__05-enable-external-repos.sh` | §3 + AUDIT-FIX-1/2 |
| `scripts/lib/packages.sh` (edit: `${DNF_SETOPT[@]}` in dnf wrapper) | `scripts__lib__packages.sh` | §2 |
| `scripts/36-akmod-guards.sh` (new) | `scripts__36-akmod-guards.sh` | §7 |
| `scripts/37-cosign-policy.sh` (new) | `scripts__37-cosign-policy.sh` | §5d |
| `system_files/etc/containers/policy.json` (new) | `system_files__etc__containers__policy.json` | §5b |
| `system_files/etc/containers/registries.d/ghcr.io-kabuki94.yaml` (new) | `system_files__etc__containers__registries.d__ghcr.io-kabuki94.yaml` | §5c |
| `system_files/sigstore/fulcio_v1.crt.pem` (new; pinned from sigstore/root-signing) | `system_files__sigstore__fulcio_v1.crt.pem` | §5d prereq |
| `system_files/sigstore/rekor.pub` (new; pinned from sigstore/root-signing) | `system_files__sigstore__rekor.pub` | §5d prereq |
| `system_files/sigstore/ublue-os.pub` (new; from ublue-os/main cosign.pub) | `system_files__sigstore__ublue-os.pub` | §5d prereq |
...
Add these lines after the existing `COPY system_files/` stage and before the final `bootc container lint` step:
...
4. For new files in subdirectories (`scripts/lib/common.sh`, `system_files/sigstore/*`), create parent directories with `New-Item -ItemType Directory -Force` before writing.
...
- scripts/05-enable-external-repos.sh: full replacement. Removed the
...
- scripts/{build,01-repos,lib/packages,05-enable-external-repos}.sh:
...
- system_files/etc/containers/registries.d/ghcr.io-kabuki94.yaml:
  use-sigstore-attachments: true.
- scripts/37-cosign-policy.sh: install policy + pinned TUF roots.
...
1. `bash -n scripts/**/*.sh` — syntax check all shell scripts.
2. `shellcheck scripts/**/*.sh` — lint; expect SC1091 on `source "lib/common.sh"` in some scripts (path resolution via `$(dirname "$0")` is fine).
3. `! grep -rnE '^\s*dnf (install|remove|swap|group)' scripts/ | grep -v '"${DNF_SETOPT\[@\]}"'` — every mutating dnf call carries the array.
4. `jq -e . system_files/etc/containers/policy.json` — policy.json parses.
