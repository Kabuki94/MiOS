...
The `-s GAPPS` flag selects Google Apps images, `-f` forces re-initialization, and the `-c`/`-v` flags explicitly set the OTA channels in case automatic resolution fails. This downloads **LineageOS 20.0 (Android 13)** GAPPS images from SourceForge.
