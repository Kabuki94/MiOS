...
MiOS implements the **OpenAI Function Calling** standard to allow FOSS-aligned AI agents (Ollama, LocalAI, LiteLLM) to natively interact with system management tools.
...
2.  **Structured CLI Output:** Core tools (e.g., `mios-update`) support a `--json` flag to return machine-readable data, eliminating interactive prompts or decorative text that breaks LLM tool-calling logic.
