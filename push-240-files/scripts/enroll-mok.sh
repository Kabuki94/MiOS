#!/usr/bin/bash
# enroll-mok.sh — CloudWS Secure Boot MOK enrollment helper.
#
# Uses mokutil throughout. sbctl is the WRONG tool for Fedora bootc
# (GRUB2+shim chain, not systemd-boot+UKI). See docs/SECUREBOOT.md.
#
# Variant-aware:
#   CloudWS-2 (ucore-hci): prefers /etc/pki/akmods/certs/akmods-ublue.der (ublue key)
#                           if /etc/pki/cloudws/mok.der absent.
#
...
pick_key() {
    ...
    # CloudWS-specific key (generated by generate-mok-key.sh)
    [[ -f /etc/pki/cloudws/mok.der ]] && { echo /etc/pki/cloudws/mok.der; return; }
    # CloudWS-2 (ucore-hci): ublue pre-signed NVIDIA kmods key
...
if [[ -z "$KEY" ]]; then
    ...
    log "  sudo scripts/generate-mok-key.sh"
    exit 2
fi
log "Using key: $KEY"
...
case "$CURRENT_STATUS" in
    ...
    conflict)
        ...
        log "  1. mokutil --delete /etc/pki/cloudws/mok.old.der  (previous key)"
